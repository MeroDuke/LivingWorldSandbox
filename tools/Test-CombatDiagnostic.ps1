...
$requiredEquipmentPatterns = @(
    ...
    'Function\s+LWS_GrabEquipment',
    'ThisAgent''s\s+"ActiveScript"\s*=\s*\$Travel_To_Safe',
...
$requiredHookPatterns = @(
    ...
    '\$SpawnUnit\s*\(\s*WeaponDrop\s*,\s*"LWS_Test_Longbow_Resource"',
    ...
    'LWS_IsEquipmentCompatible\s*\(\s*Ranger\s*,\s*RejectedWeaponDrop\s*\)\s*==\s*FALSE',
    'Ranger''s\s+"ActiveScript"\s*=\s*\$LWS_GrabEquipment',
